...
- **Regional Hostnames** — Regionalize proxied hostnames. You assign a region to a hostname through the [Regional Hostnames API](/data-localization/regional-services/get-started/) or the dashboard, and Cloudflare steers traffic for that hostname to in-region data centers. This is the most common option and is generally available. To set it up, see [Get started](/data-localization/regional-services/get-started/).

- **Regionalized Spectrum Applications** — Regionalize [Spectrum](/spectrum/) HTTP/S applications. Spectrum applications use a separate regionalization mechanism from the Regional Hostnames API, and work with both [Static IPs](/byoip/concepts/static-ips/) and [Bring Your Own IP (BYOIP)](/byoip/). To set it up, see [Regionalized Spectrum Applications](/data-localization/regional-services/spectrum-applications/).

- **Regionalized IP Bindings** — Bind a [BYOIP](/byoip/) prefix to a region so that traffic destined for those IP addresses is processed in-region. This option requires the Regional Services and Regional Services for BYOIP entitlements. To set it up, see [Regionalized IP Bindings](/data-localization/regional-services/ip-bindings/).
...
| Regionalized IP Bindings           | Regional Services for BYOIP, regionalized address maps | Entitlement required |
