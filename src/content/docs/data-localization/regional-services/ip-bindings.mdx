...
Use the `type` query parameter to filter the results by region type:

- `managed` — the standard, Cloudflare-defined regions (for example, `eu` or `us`). These are the regions listed on [Available regions and product support](/data-localization/region-support/).
- `custom` — bespoke regions configured for your account by your account team, restricting processing to a specific set of data centers when the managed regions do not meet your compliance requirements.

Results are paginated — pass the `cursor` value from a response to fetch the next page.
...
During this propagation window, your addresses remain fully available and reachable globally — there is no downtime. Traffic is served from Cloudflare's global network as usual until the binding takes effect, at which point TLS termination and processing for those addresses move into the configured region. Because traffic is never interrupted, you can create bindings ahead of a migration or compliance deadline and they will begin regionalizing once propagation completes.
