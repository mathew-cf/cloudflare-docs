...
3. **Configure the processing region.** The region is assigned by Cloudflare, so communicate and work with your account team to confirm which [region](/data-localization/region-support/) should apply to your zone and to verify it has been configured. You can use any standard region, or request a [custom region](#custom-regions) if your compliance requirements are not met by the standard regions.
...
If the [standard regions](/data-localization/region-support/) do not match your compliance requirements, you can request a custom region that restricts processing to a specific set of data centers. Custom regions are configured by your account team. To learn more about how custom regions work, refer to the [Custom regions blog post](https://blog.cloudflare.com/custom-regions/).
